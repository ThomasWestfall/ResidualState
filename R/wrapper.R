...
#' @param state.grid option to plot states over time in a grid plot.
...
plot.hydroState <- function(x, ...,
  ...
  if (is.null(siteID)) {
    ...
    if(pse.residuals == TRUE){
      ...
      # default is to plot every time series
      if(missing(ind.variable) & missing(dep.variable) & missing(dep.variable.transformed) & missing(cond.state.prob) & missing(state.grid)){

        if(any(names(x@input.data) != "ind.variable")){
        ...
        if(state.grid) plot.type = c(plot.type, "grid")
...
# Get and Plot States
#' \code{plot.user.grid}
# @export plot.user.grid
# plot.user.grid <- function(x,...,
#
