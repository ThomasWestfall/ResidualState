...
model.monthly.2 = build(input.data = data.frame(
                        ...
                        month = Residual.monthly_234201B$month,
                        dep.variable = Residual.monthly_234201B$residual,
                        ind.variable = Residual.monthly_234201B$flow),
                data.transform = 'none',
                parameters = c('a0','std.a0'),
                state.shift.parameters = c('a0','std.a0'),
                error.distribution = 'truc.normal',
...
```
## Compare 1-state and 2-state models based on negative log-likelihood
If the 2-state model has a better likelihood than the 1-state model, than we can accept the 2-state model. Since we are optimizing through minimizing the negative log-likelihood function, the model with the lower negative log-likelihood is the better model. Thus, the 2-state model can only be accepted when the negative log-likelihood is lower than the 1-state model. 
```r
...
```
-59.5713 < -50.3986, so we can accept the 2-state model is best at explaining the reality of the system. 

## Tile plot of states over time
The states are named based on the frequency of occurrence throughout the time-series. The 'normal' state is assigned to the state that occurs most often. This 'normal' is relative, where the other state will either be 'low' or 'high' depending on the 'normal' value. 
```r
# set the reference year to name the states
model = setInitialYear(model.monthly.2, 1993)
